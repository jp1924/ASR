...
class PackedWav2Vec2ForPreTraining(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        if input_lengths is not None:
            ...
            contrastive_loss_ls = list()

            for extract_feature, mask_time_indice, transformer_feature in zip(
                torch.split(extract_features, input_lengths, dim=1),
                torch.split(mask_time_indices, input_lengths, dim=1),
                torch.split(transformer_features, input_lengths, dim=1),
            ):
                quantized_features, perplexity = self.quantizer(
                    extract_feature,
                ...
                sampled_negative_indice = _sample_negative_indices(
                    mask_time_indice.cpu().numpy().shape,
                    self.config.num_negatives,
                    mask_time_indices=mask_time_indice.cpu().numpy(),
                )
                sampled_negative_indice = torch.tensor(sampled_negative_indice)

                batch_size, sequence_length, hidden_size = quantized_features.shape
                negative_quantized_features = quantized_features.view(-1, hidden_size)[
                    sampled_negative_indice.long().view(-1)
                ]
                ...
                target = ((1 - mask_time_indice.long()) * -100).transpose(0, 1).flatten()

                contrastive_loss = nn.functional.cross_entropy(logits.float(), target, reduction="sum")
                contrastive_loss_ls.append(contrastive_loss[None])

            # 7. compute diversity loss: \mathbf{L}_d
            codevector_perplexity = codevector_perplexity / len(input_lengths)
            contrastive_loss = torch.cat(contrastive_loss_ls).sum()
